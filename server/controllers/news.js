...
export const newsControllers = {
  getHeadlines: async function (req, res) {
    try {
      ...
      const { category, lang, country, page } = req.query;

      const response = await newsServices.getHeadlines({ category, lang, country, page });
  ...
  searchNews: async function (req, res) {
    try {
      const { query, lang, country, page } = req.query;

      const response = await newsServices.searchNews({ query, lang, country, page });
